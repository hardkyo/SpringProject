...
$(document).ready(function () {
	$('#id').keyup(function () {
		var output = '4자 이상 17자 이하입니다.';
		var resultview = $('#result');
		var sid = $(this).val(); // document.getElementById("id").value;
		//alert(sid.length);
		if (sid.length > 3 && sid.length < 17) {
			$.ajax({
				type: 'GET', 
				dataType: 'json',
				url : '${root}/user/idcheck.kitri',
				data: {'sid' : sid}, 
				success : function (data) { //{"count":0,"sid":"asdf"}
					if(data.count == 0){
						output = '<font color = "blue">' + data.sid + '</font>는 사용가능';
						count = 0;
					}else {
						output = '<font color = "red">' + data.sid + '</font>는 사용불가능';
					}
					resultview.empty();
	...
	$('#zipBtn').click(function() {
		window.open("${root}/user/zipsearch.kitri","zip","top=200, left=300, width=400, height=300, menubar=no, status=no, toolbar=no, location=no, scrollbars=yes");
	});
...
function join(){
	if(document.getElementById("id").value == "") {
		...
		document.joinform.action = "${root}/user/register.kitri";
 ...
 <body>
  <center>
	<h3>회원가입</h3>
	<form name ="joinform" method="post" action="${root}/user/register.kitri">
...
-->
			...
			<tr>
			 ...
			 <td class="td4">
			 <input type="text" name="id" id="id" value="" size="12">
				<div id = "result"></div>
			 </td>
...
-->
			...
			<tr>
			 ...
			 <td class="td4"><input type="text" name="zip1" id="zip1" value="" size="5" maxlength="3"> -
			 <input type="text" name="zip2" id="zip2" value="" size="5" maxlength="3">
			 <input type="button" id = "zipBtn" value="우편번호검색"></td>
			</tr>
			<tr>
